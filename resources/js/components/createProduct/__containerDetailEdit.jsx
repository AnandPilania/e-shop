...
const ContainerDetailEdit = (props) => {
    ...
    useEffect(() => {
        ...
        // récupére les types de détails de la table type_detail_products pour remplire le select id=selectdetails
        Axios.get(`http://127.0.0.1:8000/getProductDetails/${props.productId}`)
            .then(res => {

                let data = [];
                let types = [];
                // on met les types dans un array pour boucler dessus et ainsi séparer les objets par types
                res.data.objDetails.map(detail => {
                    var index = types.indexOf(detail.type);
                    if (index == -1) {
                        types.push(detail.type);
                        console.log('types  ', types);
                    }
                });
                console.log(res.data.objDetails);
                // on boucle sur les types et ensuite sur objDetails pour récupérer et stocker chaque éément du détails. Ceci prmet de reconstituer "blockDetail"
                var ndx = 0;
                var details = {};
                var detail = {};
                types.map((itemType, index) => {
                    ...
                    res.data.objDetails.map(item => {
                        if (item.type == itemType) {
                            if (ndx === 0) {
                                details.id = 'detail' + index;
                                details.typeOption = item.type;
                            }

                            detail[ndx] = item.libelle;
                            console.log(ndx);
                            ndx++;
                        }
                    });

                    Object.assign(details, { detail: detail, listTypes: listTypes });
                    console.log(details);
                    data.push(details);
                    // console.log(data);
                });
                ...
                // data.map(item => console.log(item.type));
    ...
    const refreshSelectType = (currentListType) => {
        ...
        for (let j = 0; j < blockDetail.length; j++) {
            ...
            let trueType = blockDetail[j].typeOption;
    ...
    // console.log(blockDetail);
    return (
        <div className='brd-red-1'>

            {Object.keys(blockDetail).map((option, index) =>

                <GetTypeDetailProduct
                    key={index}
                    idx={index}
                    ...
                    details={blockDetail[index]['detail']}
                    selectedType={blockDetail[index]['typeOption']}
...
export default ContainerDetailEdit;