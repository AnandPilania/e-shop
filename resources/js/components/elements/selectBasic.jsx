...
export default function SelectBasic({ list, itemSelected, setItemSelected, toggleSelect, setToggleSelect, ulUnikId, buttonUnikId }) {
    ...
    return (
        <div className="w-full relative">
            <div
                id={buttonUnikId}
                className={`flex justify-start items-center w-48 max-w-full h-10 px-2 m-0 border border-gray-300 rounded-md cursor-pointer  hover:border-gray-400 bg-white bg-no-repeat 
            ...
            <ul
                id={ulUnikId}
                className="absolute top-[46px] left-0 w-48 h-0 bg-white mb-4 transition-height duration-150 ease-in-out overflow-auto z-10 border-gray-100 border-l border-r shadow-lg rounded-md">
                {
