...
export default function Select({ list, itemSelected, setItemSelected, toggleSelect, setToggleSelect, selectValueColor, setSelectValueColor, ulUnikId }) {
    ...
    const handleChangeSelect = (name, index) => {
        let ndx = 0;
    ...
    console.log('list  ', list, ulUnikId)
    ...
    return (
        <div className="w-full relative">
            <div
                ...
                bg-right-center ${itemSelected == '' && "bg-chevron-expand"}`}
                ...
                className="absolute top-[46px] left-0 w-full h-0 bg-white mb-4 transition-height duration-150 ease-in-out overflow-auto z-10 border-gray-100 border-l border-r shadow-lg rounded-md">
                {
                    list.length > 0 && list.map((item, ndx) =>
                        <li
                            ...
                            onClick={() => handleChangeSelect(item.name, ndx)}
                        >
